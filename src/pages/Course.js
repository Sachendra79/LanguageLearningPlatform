...
//     } catch (error) {
//       console.error('Error fetching courses by search term:', error);
//     }
//   };
...
//     className='rounded-xl boder-solid border-black border-2'
//   />
//   <button onClick={handleSearch} className='bg-[#594ed3] text-[#000] ml-3 rounded-xl '>Search</button>
// </div>
...
//         {sectors.map((sector) => (
//           <Link className=' border-black border-solid border-2 px-3 bg-[#fff] rounded-xl' key={sector.sector_uuid} to={`/courses/${sector.sector_uuid}`}
//            onClick={() => setSectors([])}
//            >
//             {sector.sector_name}
//           </Link>
...
//       { loading ? (<ReactLoading type='spin'color="blue" height={60} width={60} className='loader' /> ) : (
//       sectors.map((sector) => (
...
//                 <img className='rounded-xl' src={courseimage}></img>
...
//               <Link className='course-detail border-2 border-solid border-slate-800 p-1 bg-red-700 rounded-2xl' to={`/courses/detail/${course.course_uuid}`}>View Details</Link>
//               </div>
//             ))}
//           </div>
//         </div>
...
{/* 
<div className='category-filters flex flex-wrap gap-5 text-[#000] font-semibold '>
        <Link to="/courses" onClick={() => setSectors([])} className='bg-[#fff] border-black border-solid border-2 px-2 rounded-xl'>
          All
        </Link>
        {sectors.map((sector) => (
          <Link
              ...
              sectors.map((sector) => (
                <div key={sector.sector_uuid} className="sector">
                  ...
                  <div className="featured-courses ">
                    {sector.featured_course.map((course) => (
                      <div key={course.course_uuid} className="courses">
                        <img className='rounded-xl' src={courseimage} alt={course.title}></img>
                        <h4 className='text-[#000] font-semibold'>{course.title}</h4>
                        <p className='course-author text-[#000]'>Author: {course.author.name}</p>
                        <p className='course-price text-[#b33838]'>Price: ${course.price}</p>
